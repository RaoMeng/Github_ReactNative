 ...
 * @param pageSize
 */
export const trendingRefresh = (storeName, url, pageSize, favoriteDao) => {
  return dispatch => {
    dispatch({type: actionTypes.TRENDING_REFRESH, storeName: storeName});
    let dataStore = new DataStore();
    dataStore
      .getData(url, FLAG_STORAGE.flag_trending)
      .then(response => {
        handleData(
          dispatch,
          storeName,
...
export const trendingFlush = (
  ...
  return dispatch => {
    ...
    _wrapProjectModel(
      ...
      wrapProjectModel => {
        dispatch({
          type: actionTypes.TRENDING_FLUSH_DATA,
          storeName,
          pageIndex,
          projectModels: wrapProjectModel,
        });
